  ...
  return (
    <div className="flex gap-2">
      ...
      <div className="flex-1 min-w-0">
        <span className="text-[11px] font-medium text-gray-500">{username}</span>
        <p className="text-[13px] text-gray-800 mt-0.5 leading-snug">{text}</p>
        <div className="flex items-center justify-between mt-1">
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-gray-400">{time}</span>
            <span className="text-[10px] text-gray-400">Odgovori</span>
          </div>
          <div className="flex items-center gap-0.5">
            <ChevronUp className={`w-3.5 h-3.5 ${voted ? "text-orange-500" : "text-gray-300"}`} />
            <span className="text-[11px] font-semibold text-gray-600 w-3 text-center">{votes}</span>
...
function Reply({ username, text, time, votes }: {
  ...
  return (
    <div className="ml-8 mt-2 pl-2.5 border-l-2 border-gray-100">
      <div className="flex gap-2">
        <div className="w-5 h-5 rounded-full bg-gray-100 flex items-center justify-center shrink-0">
          <User className="w-2.5 h-2.5 text-gray-400" />
        </div>
        <div className="flex-1">
          <span className="text-[10px] font-medium text-gray-500">{username}</span>
          <p className="text-[12px] text-gray-800 mt-0.5 leading-snug">{text}</p>
          <div className="flex items-center justify-between mt-1">
            <span className="text-[9px] text-gray-400">{time}</span>
            <div className="flex items-center gap-0.5">
              <ChevronUp className="w-3 h-3 text-gray-300" />
              <span className="text-[10px] font-semibold text-gray-600 w-3 text-center">{votes}</span>
...
function ProductDemo() {
  return (
    <div
      className="relative w-full max-w-[480px] rounded-xl overflow-hidden shadow-[0_8px_40px_rgba(0,0,0,0.12)] border border-[#e5e5e5]"
      style={{ aspectRatio: "9 / 11" }}
    >
      {/* Screenshot cropped to show right portion */}
      <img
        src="/demo.png"
      ...
      <div
        ...
        style={{ left: "27%", top: "28%", right: "1%", bottom: "3%" }}
      >
