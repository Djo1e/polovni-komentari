import { ConvexProvider, ConvexReactClient, useQuery, useMutation } from "convex/react";
import { useCallback, useState } from "react";
import { api } from "../../convex/_generated/api";
import { Id } from "../../convex/_generated/dataModel";
import { Drawer } from "./Drawer";
import { Comment } from "./CommentItem";
import { ShadowPortalContext } from "../context/shadow-portal";
import { saveUsername } from "../utils/username";
import { ListingInfo } from "../utils/listingInfo";
import { trackEvent } from "../utils/tracking";
import { useAuth } from "../hooks/useAuth";

type ReactHandler = (targetType: "listing" | "comment", targetId: string, emoji: string, category: "price" | "general") => void;

const convex = new ConvexReactClient(import.meta.env.VITE_CONVEX_URL as string);

interface AppProps {
  listingId: string | null;
  anonymousId: string;
  portalContainer: HTMLElement;
  initialUsername: string;
  initialIsAutoGenerated: boolean;
  listingInfo: ListingInfo | null;
}

function countAllComments(comments: Comment[]): number {
  return comments.reduce((sum, c) => sum + 1 + (c.replies?.length ?? 0), 0);
}

function CommentApp({ listingId, anonymousId, initialUsername, initialIsAutoGenerated, listingInfo }: Omit<AppProps, "portalContainer">) {
  const commentsRaw = useQuery(
    api.comments.getComments,
    listingId ? { listingId, reactorId: anonymousId } : "skip"
  );
  const isDeletedListing = !!listingId && !listingInfo;
  const listingData = useQuery(
    api.listings.getListing,
    isDeletedListing && listingId ? { listingId } : "skip"
  );
  const latestComments = useQuery(api.comments.getLatestComments);
  const voteMutation = useMutation(api.votes.vote);
  const postMutation = useMutation(api.comments.postComment);
  const reactMutation = useMutation(api.reactions.react);
  const upsertListingMutation = useMutation(api.listings.upsertListing);
  const [error, setError] = useState<string | null>(null);
  const [localVotes, setLocalVotes] = useState<Record<string, "up" | "down" | null>>({});
  const [username, setUsername] = useState(initialUsername);
  const [isAutoGenerated, setIsAutoGenerated] = useState(initialIsAutoGenerated);
  const auth = useAuth(anonymousId);

  const unreadCount = useQuery(
    api.notifications.getUnreadCount,
    auth.user ? { userId: auth.user.userId, sessionToken: auth.user.sessionToken } : "skip"
  );
  const notifications = useQuery(
    api.notifications.getNotifications,
    auth.user ? { userId: auth.user.userId, sessionToken: auth.user.sessionToken } : "skip"
  );
  const markAllReadMutation = useMutation(api.notifications.markAllRead);
  const markReadMutation = useMutation(api.notifications.markRead);

  const comments: Comment[] = (commentsRaw ?? []) as Comment[];

  const handleUsernameChange = useCallback((newUsername: string, autoGenerated: boolean) => {
    if (autoGenerated) {
      setIsAutoGenerated(true);
      saveUsername(username, true);
    } else {
      setUsername(newUsername);
      setIsAutoGenerated(false);
      saveUsername(newUsername, false);
    }
  }, [username]);

  const handleVote = useCallback(
    async (commentId: string, direction: "up" | "down") => {
      const prev = localVotes[commentId] ?? null;
      const next = prev === direction ? null : direction;
      setLocalVotes((v) => ({ ...v, [commentId]: next }));
      trackEvent("comment_vote", { direction });
      try {
        await voteMutation({
          commentId: commentId as Id<"comments">,
          voterId: anonymousId,
          direction,
        });
      } catch {
        setLocalVotes((v) => ({ ...v, [commentId]: prev }));
      }
    },
    [localVotes, voteMutation, anonymousId]
  );

  const handlePost = useCallback(
    async (text: string) => {
      if (!listingId) return;
      trackEvent("comment_post");
      if (listingInfo) {
        const { title, price, imageUrl, url } = listingInfo;
        await upsertListingMutation({ listingId, title, price, imageUrl, url });
      }
      await postMutation({
        listingId,
        text,
        authorId: anonymousId,
        username: auth.user?.displayName ?? username,
        isAutoGenerated: auth.user ? false : isAutoGenerated,
        userId: auth.user?.userId,
        sessionToken: auth.user?.sessionToken,
      });
    },
    [postMutation, upsertListingMutation, listingId, listingInfo, anonymousId, username, isAutoGenerated, auth.user]
  );

  const handleReply = useCallback(
    async (parentId: string, text: string) => {
      if (!listingId) return;
      trackEvent("comment_reply");
      if (listingInfo) {
        const { title, price, imageUrl, url } = listingInfo;
        await upsertListingMutation({ listingId, title, price, imageUrl, url });
      }
      await postMutation({
        listingId,
        text,
        authorId: anonymousId,
        parentId: parentId as Id<"comments">,
        username: auth.user?.displayName ?? username,
        isAutoGenerated: auth.user ? false : isAutoGenerated,
        userId: auth.user?.userId,
        sessionToken: auth.user?.sessionToken,
      });
    },
    [postMutation, upsertListingMutation, listingId, listingInfo, anonymousId, username, isAutoGenerated, auth.user]
  );

  const handleReact: ReactHandler = useCallback(
    async (targetType, targetId, emoji, category) => {
      trackEvent("reaction", { targetType, emoji, category });
      await reactMutation({ targetType, targetId, reactorId: anonymousId, emoji, category });
    },
    [reactMutation, anonymousId]
  );

  const handleMarkAllRead = useCallback(async () => {
    if (!auth.user) return;
    await markAllReadMutation({ userId: auth.user.userId, sessionToken: auth.user.sessionToken });
  }, [auth.user, markAllReadMutation]);

  const handleMarkRead = useCallback((id: Id<"notifications">) => {
    if (!auth.user) return;
    markReadMutation({ notificationId: id, userId: auth.user.userId, sessionToken: auth.user.sessionToken });
  }, [auth.user, markReadMutation]);

  const mergedVotes = {
    ...Object.fromEntries(
      Object.entries(localVotes).filter(([, v]) => v !== null) as [string, "up" | "down"][]
    ),
  };

  const connectionError = commentsRaw === undefined ? error : null;

  return (
    <Drawer
      listingId={listingId}
      comments={comments}
      commentCount={countAllComments(comments)}
      latestComments={latestComments ?? []}
      currentVotes={mergedVotes}
      onVote={handleVote}
      onPost={handlePost}
      onReply={handleReply}
      onReact={handleReact}
      error={connectionError}
      onRetry={() => setError(null)}
      anonymousId={anonymousId}
      username={username}
      isAutoGenerated={isAutoGenerated}
      onUsernameChange={handleUsernameChange}
      vin={listingInfo?.vin ?? null}
      boughtNewInSerbia={listingInfo?.boughtNewInSerbia ?? false}
      deletedListingData={listingData ?? null}
      auth={auth}
      unreadCount={unreadCount ?? 0}
      notifications={notifications ?? []}
      onMarkAllRead={handleMarkAllRead}
      onMarkRead={handleMarkRead}
    />
  );
}

export default function App({ portalContainer, ...props }: AppProps) {
  return (
    <ShadowPortalContext.Provider value={portalContainer}>
      <ConvexProvider client={convex}>
        <CommentApp {...props} />
      </ConvexProvider>
    </ShadowPortalContext.Provider>
  );
}
